...
class OpenGraph implements Iterator
{
   ...
   */
	static private function _parse($HTML,$URI) {
		...
		$parse_host = parse_url($page->_values['url']);
		if(!$parse_host['host']) {
			$page->_values['url'] = $URI;
		}

		$parse_image = parse_url($page->_values['image']);
		if(!$parse_image['host']) {
			$parse_host = parse_url($page->_values['url']);
			$page->_values['image'] = $parse_host['scheme'].'://'.$parse_host['host'].'/'.ltrim($parse_image['path'],'/');
		}
