...
function detect_operators($a) {
	...
	foreach($a as $word) {
		foreach($operators as $o) {
			$pos = stripos($word,$o.':');
			if($pos !== false) {
				// array_push($return,['operator' => $o,'term' => substr($word,strlen($o)+1)]);
				$return[$o] = substr($word,strlen($o)+1);
			}
		}
	}
...
function remove_operators($a,$operators = false) {
	$return    = [];

	if(!$operators) {
		$operators = detect_operators($a);
	}
	$a = explode(' ',$a);
...
function detect_objects($a) {
	...
	foreach($a as $o) {
		...
		if($is) {
			$return[$is[0]][] = $is[1]; //substr($word,strlen($o)+1);
		}
		// if($is) {
...
function highlight_links($original) {
	// $match_href = '/\.https:\/\//i';	
	// $original = preg_replace_callback($match_href,function($match){
	// 	// p($match);
	// 	return '. https://';
	// }, $original);

	/*
	// $match_href = '|\b((https?://)?([\d\w\.-]+\.[\w\.<-- (quite este punto, pero creo que vale para los .co.uk y similares)]{2,6})[^\s\]\[]*---------/?)|i';	
	*/
	$match_href = '|\b((https?://)?([\d\w\.-]+\.[\w]{2,6})[^\s\]\[]*/?)|i';	
	$original = preg_replace_callback($match_href,function($match){
		// p($match);
		if(stripos($match[1],'</a') !== false && stripos($match[1],'?q=%23') !== false){
			$match[6] = '<a target="_new" href="'.($match[0]).'>';
			// p($match);
			// return $match[0];
			// $match[5] = str_ireplace('https://mastodonshare.com/index/?q=%23','',$match[0]);
		} elseif(stripos($match[1],'<strong>') !== false){
			$l = $match[0];
			if(substr($l,-8,8) == '</strong') {
				$l .= '>';
			}
			$match[7] = '<a target="_new" href="'.strip_tags($match[1]).'">'.$l.'</a>';
		} elseif(stripos($match[1],'">') !== false){
			return $match[0];
		} else {
			$match[8] = '<a target="_new" href="'.strip_tags($match[1]).'">'.$match[1].'</a>';
		}
		return end($match).'';
		// // if($match[2]=='https://'){
		// 	p($match);
		// if(stripos($match[1],'">#') !== false){
		// 	$match[5] = '<a href="'.($match[1]);
		// 	return $match[5];
		// 	// return'<a href="'.strip_tags($match[0]).'">'.strip_tags($match[0]).'</a>';
		// } else {
		// 	return'<a href="'.strip_tags($match[1]).'">'.strip_tags($match[1]).'</a>';
		// }
		// return "¡¡".$match[0]."!!";
	}, $original);
	return $original;
} 
function highlight_hashtags($original) {
	$match_hash = '|\B#([\d\w_]+)|i';
	$original = preg_replace_callback($match_hash,function($match){
		return '<a href="'.URL.'?q=%23'.$match[1].'">'.strip_tags($match[0]).'</a>';
	}, $original);

	$match_hash = '|\B#<strong>([\d\w_]+)<\/strong>|i';
	$original = preg_replace_callback($match_hash,function($match){
		// p($match);
		return '<a href="'.URL.'?q=%23'.$match[1].'"><strong>'.strip_tags($match[0]).'</strong></a>';
	}, $original);
	return $original;
} 
...
function highlight($original,$highlight) {
	...
	// 	$highlight = mb_strtolower($highlight);
	...
	// 	OR in_array($original[$k],$highlight)
	...
	// 	if(empty($v)) continue;
	// 	// $pos = stripos($original,$v);
	// 	// if($pos !== false) {
	// 	// 	$sub = strip_tags(substr($original,$pos,strlen($v)));
	// 	// 	$original = str_ireplace($sub,'<strong>'.$sub.'</strong>',$original);
	// 	// }
	// }
	...
	if(is_string($highlight)) {
		$highlight = clean_i18n(html_entity_decode($highlight));
		// $highlight = mb_strtolower($highlight);
		$highlight = explode(' ',$highlight);
	}
	...
	if(count($highlight) > 0) {
		$original = preg_replace_callback(
			'/\b('.addslashes(implode('|',$highlight)).')\b/i',
		function($match){
			// p($match);
...
function fixes_content($original) {
	$original = preg_replace_callback('/^RT \@(.*?)\@twitter.com/i',function($match){
		return $match[0].' ';
		// p($match);
		// return 'RT <a href="https://twitter.com">@'.$match[1].'@Twitter.com</a> ';
		// return '<strong>'.$match[0].'</strong>';},$original);
	},$original);
	return $original;
}

function highlight_twitter($original) {
	$original = preg_replace_callback('/^RT \@(.*?)\@twitter.com/i',function($match){
		// return $match[0].' ';
		// p($match);
		return 'RT <a target="_new" href="https://twitter.com/@'.$match[1].'">@'.$match[1].'@twitter.com</a> ';
		// return '<strong>'.$match[0].'</strong>';},$original);
	},$original);
	return $original;
}

function clean_puntuaction($a) {
	return preg_replace("/[\p{P}]/",' ',$a);
}

function clean_i18n($a) {
	return preg_replace("/[^\p{L}\p{N}\s]/",'',$a);
}
...
function detect_sort($sort = false) {
	$list = list_sort();
	if(isset($list[$sort])) {
		return $sort;
	}
